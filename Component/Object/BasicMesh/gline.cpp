...
{
    _vertBegin.Position = p1;
    _vertBegin.Color    = c1;

    _vertEnd.Position   = p2;
    _vertEnd.Color      = c2;
...
void GLine::GLBufferInitialize()
{
    ...
    /* 填充数据 */
    gl->glBindVertexArray(VAO);
    gl->glBufferSubData(GL_ARRAY_BUFFER,              0, ColorVert_SIZE, &_vertEnd);
    gl->glBufferSubData(GL_ARRAY_BUFFER, ColorVert_SIZE, ColorVert_SIZE, &_vertBegin);
