...
LinMotion::GetAvoidObstCurvesFunction(KUKA_IIWA_MODEL* robot, double Padding, const double Max_Vel_Limit, const double Max_Acc_Limit)
{
    ...
    auto SimpleLineFunc = GetScrewLineFunction(T_ini, goals_);
    ...
    double scaler = (T_ini.block(0, 3, 3, 1) - goals_.block(0, 3, 3, 1)).norm();
...
LinMotion::GetAvoidLimitationCurvesFunction(KUKA_IIWA_MODEL* robot, const double Max_Vel_Limit, const double Max_Acc_Limit)
{
    ...
    auto LineFunc = GetScrewLineFunction(T_ini, goals_);
...
/* DELETE THIS PART
void LineMotion::addObstacle(const string & name, BallObst && obst)
{
    deleteObstacle(name);
    obst_Dict.insert(make_pair(name, forward<BallObst>(obst)));
}

void LineMotion::updateObstacle(const string & name, Vec3d pos_new)
{
...
void LineMotion::deleteObstacle(const string & name)
{
    if(obst_Dict.count(name))
    {
