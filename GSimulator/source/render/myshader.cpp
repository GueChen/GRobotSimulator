...
void MyShader::SetRhiDevice(std::shared_ptr<IRhiDevice> rhi_device)
{
    ...
    if (!init_) {
        ...
        for (int i = 0; i < num_uniforms; ++i) {
            ...
            GLint  values[3];
            GL()->glGetProgramResourceiv(programId(), GL_UNIFORM, i, 3, properties, 3, nullptr, values);

            GLint name_length = values[0];
            std::unique_ptr<char[]> name_buffer(new char[name_length]);
            
            GL()->glGetProgramResourceName(programId(), GL_UNIFORM, i, name_length, nullptr, name_buffer.get());
#ifdef _DEBUG
